...
const MyBookings = () => {
  ...
  const tableBookings = bookings.filter(b => b.seat !== null);
    ...
    <div className="min-h-screen bg-slate-50 py-12">
      <div className="container mx-auto px-4 max-w-5xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6 animate-fade-in">
          <div>
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">My Table Bookings</h1>
            <p className="text-slate-500 mt-2 text-lg font-medium">Manage your reserved seats and upcoming dining sessions.</p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-white rounded-[2rem] p-16 text-center border border-red-100 shadow-sm flex flex-col items-center">
            ...
            <p className="text-slate-500 max-w-sm mb-8">We couldn't load your bookings. Please try again later.</p>
          </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-8">
                  ...
                  <div className="flex-grow space-y-3">
                    <div className="flex flex-wrap items-center gap-3">
                      <h3 className="text-xl font-bold text-slate-900 uppercase tracking-tighter">Reservation #{booking.id}</h3>
                      <span className={`px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${getStatusColor(booking.status)}`}>
                        {activeTab === 'upcoming' || booking.status?.toUpperCase() === 'CONFIRMED' ? 'Booked' : booking.status}
                      </span>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-slate-500 font-medium text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span>{booking.booking_time || booking.start_time}</span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-[5rem] p-20 text-center border border-slate-100 shadow-sm flex flex-col items-center justify-center shadow-xl shadow-slate-200/50">
            ...
            <h2 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">No {activeTab} bookings</h2>
            <p className="text-slate-500 max-w-sm mx-auto text-lg leading-relaxed">
              Explore our seating chart and pick your favorite corner for an exquisite dining experience.
            </p>
            <Link 
              to="/seats" 
